						...
						int trigger)
{
	...
	/* This part needs to be executed always for OMAP34xx and OMAP44xx */
	if (cpu_is_omap34xx() || cpu_is_omap44xx() ||
...
static void omap_gpio_save_context(struct device *dev)
{
	...
	if (bank->method == METHOD_GPIO_24XX) {
		bank->gpio_context.irqenable0 =
			__raw_readl(bank->base + OMAP24XX_GPIO_IRQENABLE1);
		bank->gpio_context.irqenable1 =
			__raw_readl(bank->base + OMAP24XX_GPIO_IRQENABLE2);
		bank->gpio_context.waken0 =
		...
		bank->gpio_context.irqenable0 =
			__raw_readl(bank->base + OMAP4_GPIO_IRQSTATUSSET0);
		bank->gpio_context.irqenable1 =
			__raw_readl(bank->base + OMAP4_GPIO_IRQSTATUSSET1);
		bank->gpio_context.waken0 =
			__raw_readl(bank->base + OMAP4_GPIO_IRQWAKEN0);
		bank->gpio_context.waken1 =
			__raw_readl(bank->base + OMAP4_GPIO_IRQWAKEN1);
		bank->gpio_context.ctrl =
...
static void omap_gpio_restore_context(struct device *dev)
{
	...
	if (bank->method == METHOD_GPIO_24XX) {
		__raw_writel(bank->gpio_context.irqenable0,
				bank->base + OMAP24XX_GPIO_IRQENABLE1);
		__raw_writel(bank->gpio_context.irqenable1,
				bank->base + OMAP24XX_GPIO_IRQENABLE2);
		__raw_writel(bank->gpio_context.waken0,
		...
		__raw_writel(0xffffffff, bank->base + OMAP4_GPIO_IRQSTATUSCLR1);
		__raw_writel(bank->gpio_context.irqenable1,
				bank->base + OMAP4_GPIO_IRQSTATUSSET1);
		__raw_writel(bank->gpio_context.waken0,
				bank->base + OMAP4_GPIO_IRQWAKEN0);
		__raw_writel(bank->gpio_context.waken1,
				bank->base + OMAP4_GPIO_IRQWAKEN1);
		__raw_writel(bank->gpio_context.ctrl,
